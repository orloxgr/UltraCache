...
class Ultra_Cache_WP
{
    ...
    {
        $files = array(
            ultracache_plugin_dir('includes/class-ultra-cache-engine.php'),
            ultracache_plugin_dir('includes/class-media-converter.php'),
            ultracache_plugin_dir('includes/class-object-cache-manager.php'),
        );
    ...
    private function load_dependency_file($file)
    {
        ...
        ultracache_request_profile_checkpoint('dependency_load_start', array('file' => basename((string) $file)));
        require_once $file;
        ultracache_request_profile_checkpoint('dependency_load_end', array('file' => basename((string) $file)));
    ...
    private function load_rest_api_dependency()
    {
        ...
        return $this->load_dependency_file(ultracache_plugin_dir('includes/class-rest-api.php'));
    }
    ...
    private function load_wp_cli_dependency()
    {
        ...
        return $this->load_dependency_file(ultracache_plugin_dir('includes/class-wp-cli.php'));
    }
    ...
    {
        $dir = dirname($target);
        if (!file_exists($dir) && !ultracache_safe_mkdir($dir, 0755, true, $context . ' mkdir') && !file_exists($dir)) {
            return false;
        }

        $tmp = trailingslashit($dir) . '.' . wp_basename($target) . '.tmp-' . wp_generate_password(8, false, false);
        if (false === ultracache_safe_file_put_contents($tmp, $contents, LOCK_EX, $context . ' tmp')) {
            ultracache_safe_unlink($tmp, $context . ' tmp cleanup');
            return false;
        }

        if (!ultracache_safe_rename($tmp, $target, $context . ' rename')) {
            ultracache_safe_unlink($tmp, $context . ' rename cleanup');
            return false;
        }
...
return array(
    ...
    public static function persist_dashboard_settings(array $settings)
    {
        ...
        if ($secret_change_requested && (!current_user_can('manage_options') || !current_user_can('activate_plugins'))) {
            return new WP_Error(
                'ultracache_secret_constant_permission_denied',
                self::maybe_translate('Updating wp-config.php secrets requires manage_options and activate_plugins permissions.')
            );
        }
    ...
    private static function get_frontend_compression_probe_status($allow_live_probe = false)
    {
        ...
        $probe_url = add_query_arg('ultracache_probe_compression', (string) time(), $probe_base);
        $encodings = array(
            'brotli' => 'br',
            'gzip'   => 'gzip',
        );
