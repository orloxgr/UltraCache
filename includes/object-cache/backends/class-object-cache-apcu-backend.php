...
final class Ultra_Cache_Object_Cache_APCu_Backend extends Ultra_Cache_Object_Cache_Abstract_Backend {
	...
	protected function write_persistent_payload($key, $group, $payload, $expire) {
		if ($this->context->write_apcu_payload($key, $group, $payload, (int) $expire)) {
	...
	public function add($key, $data, $group = 'default', $expire = 0) {
		...
		$status = $this->context->add_apcu_payload($key, $group, $payload, (int) $expire);
		if ('stored' === $status) {
			$this->clear_runtime_dirty($key, $group);
			$this->context->runtime_set($key, $group, $data);
			return true;
	...
	public function replace($key, $data, $group = 'default', $expire = 0) {
		...
		$status = $this->context->replace_apcu_payload($key, $group, $payload, (int) $expire);
		if ('stored' === $status) {
			$this->clear_runtime_dirty($key, $group);
			$this->context->runtime_set($key, $group, $data);
			return true;
	...
	private function mutate_numeric($key, $offset, $group, $decrement) {
		...
		$result = $this->context->mutate_apcu_numeric_payload(
			$key,
			$group,
			(int) $offset,
			(bool) $decrement,
	...
	public function delete($key, $group = 'default') {
		$normalized_group = $this->context->normalize_group($group);
		$normalized_key = $this->context->normalize_key($key);
		$deleted = parent::delete($normalized_key, $normalized_group);
		if ($deleted) {
			$this->clear_runtime_dirty($normalized_key, $normalized_group);
		}
		return $deleted;
	}
