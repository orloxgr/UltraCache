...
(function (window) {
	...
	const PERFORMANCE_PROFILES = {
		...
		safe: { label: __("Safe", 'ultracache'), description: __("Safe baseline with frontend JavaScript manipulation disabled. Object Cache is enabled automatically with Redis/APCu/SQLite/Disk detection. User-maintained exclusions and visible lists are preserved.", 'ultracache'), patch: {
			...
			varnishHtmlTtlMinutes: 1440,
			varnishStaleWhileRevalidateSeconds: 0,
			varnishRefillAfterTargetedInvalidation: false,
			varnishWarmDuringManualWarmup: false,
			varnishVerifyRefillHit: false,
			varnishRefreshAheadEnabled: false,
			varnishRefreshAheadThresholdPercent: 85,
			varnishRefreshAheadMaxPages: 5,
		...
		aggressive: { label: __("Aggressive", 'ultracache'), description: __("Aggressive profile enables Balanced JavaScript with native Defer as the base strategy, targeted delay modules, aggressive CSS bundling, Warm affected pages after save, Apache Static HTML Delivery, and the unified 0.05 second fallback. Scan Browser Runtime Errors is recommended to build speed-first Defer Instead fixes before compatibility exclusions. Object Cache is auto-detected. User-maintained exclusions and visible lists are preserved.", 'ultracache'), patch: {
			pageCacheEnabled: true,
			purgeAfterCoreUpdatesEnabled: true,
			purgeAfterPluginUpdatesEnabled: true,
			purgeAfterThemeUpdatesEnabled: true,
