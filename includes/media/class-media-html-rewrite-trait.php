...
trait Ultra_Cache_Media_Html_Rewrite_Trait
{
		...
		/**
		 * Rewrite only UltraCache-owned optimized image URL values in returned HTML.
		 *
		 * This callback is used by content/render filters, so it must not escape the
		 * whole HTML document. Instead, every value generated by UltraCache is first
		 * validated as an optimized `/uc-images/` URL and escaped for its specific
		 * context before being inserted back into the original HTML.
		 *
		 * @param string $html Existing frontend HTML.
		 * @return string HTML with only validated UltraCache image URL replacements.
		 */
