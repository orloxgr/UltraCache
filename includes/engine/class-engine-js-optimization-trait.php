...
if (!trait_exists('Ultra_Cache_Engine_JS_Optimization_Trait')) {
    trait Ultra_Cache_Engine_JS_Optimization_Trait
    {
        ...
        private function apply_safe_deferred_js_bundle_to_html($html, array $settings = array())
        {
            if (empty($settings['js_bundle']) || empty($settings['defer_js']) || is_admin() || !is_string($html) || '' === $html || false === stripos($html, '<script')) {
                return $html;
            }

            $diagnostics = $this->get_empty_safe_deferred_js_bundle_diagnostics($settings);

            if (!preg_match_all('/<script\b[^>]*\bsrc\s*=\s*(["\'])(.*?)\1[^>]*>\s*<\/script>/is', $html, $matches, PREG_SET_ORDER | PREG_OFFSET_CAPTURE)) {
                $diagnostics['message'] = 'No external script tags were found in the final HTML.';
                $this->persist_safe_deferred_js_bundle_diagnostics($diagnostics);
                return $html;
            }

            $script_inline_handles = $this->collect_inline_script_handle_markers_for_js_bundle($html);
            $records = array();
            foreach ($matches as $index => $match) {
                ...
                if ('' === $handle) {
                    $handle = $this->extract_attribute_from_html_tag($tag, 'id');
                }

                $evaluation = $this->evaluate_safe_deferred_js_bundle_candidate($handle, $src, $tag, $settings, $script_inline_handles);
                $reason = isset($evaluation['reason']) ? (string) $evaluation['reason'] : 'unknown';
                $eligible = !empty($evaluation['eligible']);
                $deferred = !empty($evaluation['deferred']);
                $diagnostics['scriptsScanned']++;
                if ($deferred) {
                    $diagnostics['deferredScripts']++;
                }
                if ($eligible) {
                    $diagnostics['eligibleScripts']++;
                }
                if (!isset($diagnostics['reasonCounts'][$reason])) {
                    $diagnostics['reasonCounts'][$reason] = 0;
                }
                $diagnostics['reasonCounts'][$reason]++;
                if (count($diagnostics['samples']) < 50) {
                    $path = (string) wp_parse_url($src, PHP_URL_PATH);
                    $diagnostics['samples'][] = array(
                ...
                $records[] = array(
                    ...
                    'tag'       => $tag,
                    'offset'    => (int) $offset,
                    'end'       => (int) $offset + strlen($tag),
                    'src'       => $src,
                    'handle'    => $handle,
                    'candidate' => (bool) $eligible,
                    'reason'    => (string) $reason,
                );
            }

            if (empty($records)) {
                $diagnostics['message'] = 'No usable external script records were found in the final HTML.';
                $this->persist_safe_deferred_js_bundle_diagnostics($diagnostics);
                return $html;
            }
            ...
            foreach ($records as $record) {
                if (empty($record['candidate'])) {
                    if (count($current) >= 2) {
                        $runs[] = $current;
                    } elseif (1 === count($current)) {
                        $diagnostics['singleEligibleScripts']++;
                    }
                    $current = array();
                    $last_end = null;
                    continue;
                ...
                if (empty($current)) {
                    $current[] = $record;
                    $last_end = (int) $record['end'];
                    continue;
                }

                $between = substr($html, (int) $last_end, max(0, (int) $record['offset'] - (int) $last_end));
            ...
            $diagnostics['eligibleGroups'] = count($runs);
            if (empty($runs)) {
                $diagnostics['message'] = $diagnostics['eligibleScripts'] > 0
                    ? 'No bundle generated. Eligible scripts were isolated and not consecutive.'
                    : 'No bundle generated. No eligible safe deferred scripts were found.';
                $this->persist_safe_deferred_js_bundle_diagnostics($diagnostics);
                return $html;
            }
            ...
            $replacements = array();
            foreach ($runs as $run) {
                $bundle = $this->build_safe_deferred_js_bundle_from_run($run);
                if (empty($bundle['url']) || empty($bundle['file']) || empty($bundle['count'])) {
                    $diagnostics['bundleBuildFailures']++;
                    continue;
                }
                ...
                $end = (int) $last['end'];
                $src = esc_url($bundle['url']);
                $count = (int) $bundle['count'];
                $bytes = (int) ($bundle['bytes'] ?? 0);
                $tag = '<script id="ucwp-safe-deferred-js-bundle-' . esc_attr(substr(md5((string) $bundle['file']), 0, 8)) . '" src="' . $src . '" defer data-ucwp-js-bundle="safe-deferred" data-ucwp-js-bundle-count="' . esc_attr((string) $count) . '" data-ucwp-js-bundle-bytes="' . esc_attr((string) $bytes) . '"></script>';
                $replacements[] = array('start' => $start, 'end' => $end, 'replacement' => $tag);
                $diagnostics['bundlesGenerated']++;
                $diagnostics['bundledScripts'] += $count;
                $diagnostics['bundleBytes'] += $bytes;
                if (count($diagnostics['bundles']) < 20) {
                    $diagnostics['bundles'][] = array(
                        'url' => (string) $bundle['url'],
                        'file' => basename((string) $bundle['file']),
                        'count' => $count,
                        'bytes' => $bytes,
                    );
                }
            ...
            if (empty($replacements)) {
                $diagnostics['message'] = 'No bundle generated. Eligible groups were found, but bundle files could not be built safely.';
                $this->persist_safe_deferred_js_bundle_diagnostics($diagnostics);
                return $html;
            }
            ...
            foreach ($replacements as $replacement) {
                ...
                $out .= substr($html, $last, $start - $last);
                $out .= (string) $replacement['replacement'];
                $last = $end;
            }
            $out .= substr($html, $last);

            $diagnostics['message'] = sprintf(
                'Generated %d safe deferred JS bundle(s) from %d script(s), %s total.',
                (int) $diagnostics['bundlesGenerated'],
                (int) $diagnostics['bundledScripts'],
                function_exists('size_format') ? size_format((int) $diagnostics['bundleBytes']) : ((int) $diagnostics['bundleBytes'] . ' bytes')
            );
            $this->persist_safe_deferred_js_bundle_diagnostics($diagnostics);
        ...
        private function collect_inline_script_handle_markers_for_js_bundle($html)
        {
            $handles = array();
            ...
            if (!preg_match_all('/<script\b(?![^>]*\bsrc\s*=)[^>]*\bid\s*=\s*(["\'])([^"\']+)\1[^>]*>/is', $html, $matches)) {
                return $handles;
            }

            foreach ((array) ($matches[2] ?? array()) as $id) {
                $id = strtolower(trim((string) $id));
                if ('' === $id) {
                    continue;
                }
                if (preg_match('/^(.+?)-js-(?:before|after|extra|translations)$/', $id, $m)) {
        ...
        private function get_empty_safe_deferred_js_bundle_diagnostics(array $settings = array())
        {
            $include = $this->get_js_bundle_include_fragments($settings);
            $exclude = $this->get_js_bundle_exclude_fragments($settings);
            return array(
                ...
                'includePatternCount' => count($include),
                'excludePatternCount' => count($exclude),
                'reasonCounts' => array(),
                'samples' => array(),
                'bundles' => array(),
                'message' => '',
        ...
        private function persist_safe_deferred_js_bundle_diagnostics(array $diagnostics)
        {
            ...
            update_option('ucwp_last_js_bundle_diagnostics', $diagnostics, false);
        }

        private function evaluate_safe_deferred_js_bundle_candidate($handle, $src, $tag, array $settings, array $inline_handles = array())
        {
            $handle = (string) $handle;
            $src = trim((string) $src);
            $tag = (string) $tag;
            ...
            if ('' === $src || '' === $tag) {
                $result['reason'] = 'empty_src_or_tag';
                return $result;
            }

            $tag_lc = strtolower($tag);
            $is_deferred = (false !== strpos($tag_lc, ' defer') || false !== strpos($tag_lc, 'data-wp-strategy="defer"') || false !== strpos($tag_lc, "data-wp-strategy='defer'"));
            $result['deferred'] = (bool) $is_deferred;
            if (!$is_deferred) {
                $result['reason'] = 'not_deferred';
                return $result;
            }
            if (false !== strpos($tag_lc, ' async') || false !== strpos($tag_lc, 'type="module"') || false !== strpos($tag_lc, "type='module'") || false !== strpos($tag_lc, ' nomodule') || false !== strpos($tag_lc, ' integrity=')) {
                $result['reason'] = 'async_module_nomodule_or_integrity';
                return $result;
            }

            if (!$this->is_local_js_bundle_source($src)) {
                $result['reason'] = 'non_local_or_third_party';
                return $result;
            }

            $path = (string) wp_parse_url($src, PHP_URL_PATH);
            $path_lc = strtolower($path);
            if ('' === $path || false === substr_compare($path_lc, '.js', -3)) {
                $result['reason'] = 'not_js_file';
                return $result;
            }
            if (false !== strpos($path_lc, '/wp-includes/js/') || false !== strpos($path_lc, '/wp-admin/')) {
                $result['reason'] = 'wp_core_or_admin_script';
                return $result;
            }
            if (false !== strpos($path_lc, '/cache/ultracache/js-bundles/')) {
                $result['reason'] = 'already_generated_bundle';
                return $result;
            }

            $handle_key = strtolower(trim(preg_replace('/-js$/', '', (string) $handle)));
            if ('' !== $handle_key && isset($inline_handles[$handle_key])) {
                $result['reason'] = 'inline_before_after_dependency';
                return $result;
            }

            if ($this->is_script_absolute_defer_blocking($handle, $src, $tag, $settings) || $this->is_script_force_blocking($handle, $src, $tag, $settings) || $this->is_script_user_defer_excluded($handle, $src, $settings) || $this->is_script_safe_stage_excluded($handle, $src, $tag, $settings)) {
                $result['reason'] = 'protected_by_defer_safeguard';
                return $result;
            }

            if ($this->script_matches_fragment_list($handle, $src, $this->get_js_bundle_exclude_fragments($settings))) {
                $result['reason'] = 'bundle_exclude_pattern';
                return $result;
            }

            $include = $this->get_js_bundle_include_fragments($settings);
            if (!empty($include) && !$this->script_matches_fragment_list($handle, $src, $include)) {
                $result['reason'] = 'bundle_include_miss';
                return $result;
            }

            $file = $this->resolve_local_js_bundle_source_file($src);
            if ('' === $file || !is_readable($file) || !is_file($file)) {
                $result['reason'] = 'source_file_missing_or_unreadable';
                return $result;
            }
        ...
        private function is_safe_deferred_js_bundle_candidate($handle, $src, $tag, array $settings, array $inline_handles = array())
        {
            $evaluation = $this->evaluate_safe_deferred_js_bundle_candidate($handle, $src, $tag, $settings, $inline_handles);
            return !empty($evaluation['eligible']);
        }

        private function get_js_bundle_include_fragments(array $settings = array())
        {
            $list = array();
            if (isset($settings['js_bundle_include_list']) && is_array($settings['js_bundle_include_list'])) {
                $list = array_merge($list, $settings['js_bundle_include_list']);
            }
        ...
        private function get_js_bundle_exclude_fragments(array $settings = array())
        {
            $list = array();
            if (isset($settings['js_bundle_exclude_list']) && is_array($settings['js_bundle_exclude_list'])) {
                $list = array_merge($list, $settings['js_bundle_exclude_list']);
            }
            if (isset($settings['defer_js_exclude_list']) && is_array($settings['defer_js_exclude_list'])) {
        ...
        private function is_local_js_bundle_source($src)
        {
            ...
            $url_host = wp_parse_url($src, PHP_URL_HOST);
            if ('' === (string) $url_host) {
                return true;
            }
            ...
            return ($url_host === strtolower((string) $home_host) || $url_host === strtolower((string) $content_host));
        }

        private function resolve_local_js_bundle_source_file($src)
        {
            $src = html_entity_decode(trim((string) $src), ENT_QUOTES | ENT_HTML5);
            $path = (string) wp_parse_url($src, PHP_URL_PATH);
            if ('' === $path) {
            ...
            $path = rawurldecode($path);
            if ('' !== $content_path && 0 === strpos($path, rtrim($content_path, '/') . '/')) {
                $relative = ltrim(substr($path, strlen(rtrim($content_path, '/'))), '/');
                $file = trailingslashit(WP_CONTENT_DIR) . $relative;
            } else {
                $file = trailingslashit(ABSPATH) . ltrim($path, '/');
            }
            ...
            $content_root = function_exists('wp_normalize_path') ? wp_normalize_path(WP_CONTENT_DIR) : str_replace('\\', '/', WP_CONTENT_DIR);
            if (0 !== strpos($file, $root) && 0 !== strpos($file, $content_root)) {
                return '';
            }
        ...
        private function build_safe_deferred_js_bundle_from_run(array $run)
        {
            $parts = array();
            $sources = array();
            $total = 0;
            ...
            foreach ($run as $record) {
                ...
                $content = '';
                if (is_object($filesystem) && method_exists($filesystem, 'get_contents')) {
                    $content = (string) $filesystem->get_contents($file);
                }
                if ('' === $content) {
                ...
                $parts[] = "\n;/* UltraCache JS bundle source: " . str_replace('*/', '* /', (string) wp_parse_url($src, PHP_URL_PATH)) . " */\n" . rtrim($content) . "\n;";
            }
            ...
            $filename = 'bundle-safe-deferred-' . $hash . '.js';
            $file = trailingslashit($dir) . $filename;
            if (!is_file($file) || filesize($file) !== strlen($payload)) {
                if (!$this->write_cache_variant_atomically($file, $payload)) {
                    return array();
                }
            }
            ...
            return array(
                'file'  => $file,
                'url'   => content_url('cache/ultracache/js-bundles/' . rawurlencode($filename)),
                'count' => count($parts),
                'bytes' => filesize($file),
            );
        }
