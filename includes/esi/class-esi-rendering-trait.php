...
trait Ultra_Cache_Engine_ESI_Rendering_Trait
{
    /**
     ...
     * @return string
     */
    public function apply_esi_template_output_buffer($html)
    {
        if (
            ...
            $diagnostics = class_exists('Ultra_Cache_ESI_Registry')
                && method_exists(Ultra_Cache_ESI_Registry::instance(), 'get_template_buffer_diagnostics')
                ? Ultra_Cache_ESI_Registry::instance()->get_template_buffer_diagnostics()
                : array();
     ...
     * @return string
     */
    private function apply_esi_placeholders_before_cache_store($html)
    {
        if (!is_string($html) || '' === $html || !function_exists('ultracache_esi_is_enabled') || !ultracache_esi_is_enabled()) {
            return $html;
        }
        ...
        $html = $this->neutralize_untrusted_esi_markup($html);
        $max_fragments = function_exists('ultracache_esi_get_max_parent_fragments')
            ? ultracache_esi_get_max_parent_fragments()
            : 32;
        $max_directive_bytes = function_exists('ultracache_esi_get_max_parent_directive_bytes')
            ? ultracache_esi_get_max_parent_directive_bytes()
            : 65536;
        $emitted_fragments = 0;
        ...
        $rewritten = preg_replace_callback(
            ...
            function ($matches) use (&$emitted_fragments, &$emitted_directive_bytes, $max_fragments, $max_directive_bytes) {
                ...
                $decoded = Ultra_Cache_ESI_Registry::instance()->decode_context_token($token);
                ...
                $onerror = 'private' === $scope
                    && function_exists('ultracache_esi_private_onerror_is_enabled')
                    && ultracache_esi_private_onerror_is_enabled()
                    ? ' onerror="continue"'
                    : '';
     ...
     * @return array
     */
    private function get_esi_parent_metadata_from_html($html)
    {
        $html = (string) $html;
        ...
        $max_fragments = function_exists('ultracache_esi_get_max_parent_fragments') ? ultracache_esi_get_max_parent_fragments() : 32;
     ...
     * @return string
     */
    private function neutralize_untrusted_esi_markup($html)
    {
