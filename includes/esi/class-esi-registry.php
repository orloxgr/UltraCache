<?php
/**
 * Registered public/private ESI fragments and signed context handling.
 *
 * @package UltraCache
 */
...
final class Ultra_Cache_ESI_Registry
{
    /** @var Ultra_Cache_ESI_Registry|null */
     ...
     *
     * @return Ultra_Cache_ESI_Registry
     */
    ...
    public function create_context_token($fragment_id, array $context = array())
    {
        ...
        $payload = array(
            ...
            'ctx' => $context,
            'loc' => function_exists('ultracache_esi_get_current_locale')
                ? ultracache_esi_get_current_locale()
        ...
        $signature = hash_hmac('sha256', $encoded, $this->get_signing_key());
    ...
    public function decode_context_token($token)
    {
        ...
        $expected_signature = hash_hmac('sha256', $encoded, $this->get_signing_key());
        ...
        if (!is_string($locale) || strlen($locale) < 2 || strlen($locale) > 32) {
            $locale = function_exists('ultracache_esi_get_current_locale')
                ? ultracache_esi_get_current_locale()
                : (function_exists('get_locale') ? (string) get_locale() : 'en_US');
        }
    ...
    public function get_definition_summary($fragment_id)
    {
        ...
        return array(
            ...
            'privateTransportDeclared' => 'private' === (string) ($definition['scope'] ?? '')
                && function_exists('ultracache_esi_private_definition_transport_is_declared')
    ...
    /**
     * Return the site-specific signing key.
     *
     * @return string
     */
    private function get_signing_key()
    {
        ...
        return hash('sha256', wp_salt('auth') . '|ultracache-esi-v1|' . $blog_id . '|' . home_url('/'));
    }
