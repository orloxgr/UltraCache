<?php
/**
 * Registered public/private Varnish ESI fragments and signed context handling.
 *
 * @package UltraCache
 */
...
final class Ultra_Cache_Varnish_ESI_Registry
{
    /** @var Ultra_Cache_Varnish_ESI_Registry|null */
     ...
     *
     * @return Ultra_Cache_Varnish_ESI_Registry
     */
    ...
    public function register($fragment_id, array $args)
    {
        ...
        if ('' === $fragment_id) {
            return new WP_Error('ultracache_esi_invalid_fragment_id', __('The ESI fragment ID is invalid.', 'ultracache'));
        }
        if (isset($this->fragments[$fragment_id])) {
            return new WP_Error('ultracache_esi_fragment_already_registered', __('The ESI fragment ID is already registered.', 'ultracache'));
        }

        $renderer = $args['renderer'] ?? null;
        if (!is_callable($renderer)) {
            return new WP_Error('ultracache_esi_invalid_renderer', __('The ESI fragment renderer must be callable.', 'ultracache'));
        }
        $scope = strtolower(trim((string) ($args['scope'] ?? 'public')));
        if (!in_array($scope, array('public', 'private'), true)) {
            return new WP_Error('ultracache_esi_scope_not_supported', __('The ESI fragment scope must be public or private.', 'ultracache'));
        }
        ...
        if ('private' === $scope && empty($cookie_names) && empty($cookie_prefixes)) {
            return new WP_Error('ultracache_esi_private_cookie_allowlist_required', __('Private ESI fragments require at least one allowed cookie name or prefix.', 'ultracache'));
        }
        if ('public' === $scope) {
            $cookie_names = array();
        ...
        if ('private' === $scope && !is_string($fallback)) {
            return new WP_Error(
                'ultracache_esi_private_fallback_must_be_static',
                __('Private ESI fragment fallback must be static HTML.', 'ultracache')
            );
        }
        if (!is_callable($fallback) && !is_string($fallback)) {
            return new WP_Error('ultracache_esi_invalid_fallback', __('The ESI fragment fallback must be callable or a string.', 'ultracache'));
        }
        ...
        $definition = array(
            ...
            'max_render_ms'            => max(10, min(5000, absint($args['max_render_ms'] ?? 2000))),
        ...
        $filtered = apply_filters('ultracache_esi_fragment_definition', $definition, $fragment_id, $args);
        if (!is_array($filtered) || !isset($filtered['renderer']) || !is_callable($filtered['renderer'])) {
            return new WP_Error('ultracache_esi_invalid_filtered_definition', __('The filtered ESI fragment definition is invalid.', 'ultracache'));
        }
        ...
        if (!is_callable($filtered['fallback']) && !is_string($filtered['fallback'])) {
            return new WP_Error('ultracache_esi_invalid_filtered_fallback', __('The filtered ESI fragment fallback is invalid.', 'ultracache'));
        }
        $filtered['max_output_bytes'] = max(1024, min(1048576, absint($filtered['max_output_bytes'] ?? $definition['max_output_bytes'])));
        $filtered['max_render_ms'] = max(10, min(5000, absint($filtered['max_render_ms'] ?? $definition['max_render_ms'])));
    ...
    public function normalize_context(array $definition, array $context)
    {
        ...
        if (empty($allowed_keys)) {
            return empty($context)
                ? array()
                : new WP_Error('ultracache_esi_context_not_allowed', __('This ESI fragment does not accept context values.', 'ultracache'));
        }

        if (count($context) > min(12, count($allowed_keys))) {
            return new WP_Error('ultracache_esi_context_too_many_values', __('The ESI fragment context contains too many values.', 'ultracache'));
        }
        ...
        foreach ($context as $key => $value) {
            ...
            if ('' === $key || $key !== $raw_key || !isset($allowed_keys[$key])) {
                return new WP_Error('ultracache_esi_context_key_not_allowed', __('The ESI fragment context contains an unsupported key.', 'ultracache'));
            }
            ...
            if (is_float($value)) {
                if (!is_finite($value)) {
                    return new WP_Error('ultracache_esi_context_value_invalid', __('ESI fragment context values must be scalar.', 'ultracache'));
                }
            ...
            if (is_string($value)) {
                ...
                if (strlen($value) > $max_value_bytes) {
                    return new WP_Error('ultracache_esi_context_value_too_long', __('An ESI fragment context value is too long.', 'ultracache'));
                }
                $normalized[$key] = $value;
                continue;
            }
        ...
        if (!is_string($encoded_context) || strlen($encoded_context) > $max_context_bytes) {
            return new WP_Error('ultracache_esi_context_too_large', __('The ESI fragment context exceeds its configured size limit.', 'ultracache'));
        }
    ...
    public function create_context_token($fragment_id, array $context = array())
    {
        ...
        if (null === $definition) {
            return new WP_Error('ultracache_esi_fragment_not_registered', __('The ESI fragment is not registered.', 'ultracache'));
        }
        ...
        $payload = array(
            ...
            'ctx' => $context,
            'loc' => function_exists('ultracache_varnish_esi_get_current_locale')
                ? ultracache_varnish_esi_get_current_locale()
                : (function_exists('get_locale') ? (string) get_locale() : 'en_US'),
            'lng' => function_exists('ultracache_varnish_esi_get_current_language')
                ? ultracache_varnish_esi_get_current_language()
                : '',
        );
        $json = wp_json_encode($payload, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        if (!is_string($json) || '' === $json) {
            return new WP_Error('ultracache_esi_context_encode_failed', __('The ESI fragment context could not be encoded.', 'ultracache'));
        }

        $encoded = $this->base64url_encode($json);
        $signature = ultracache_internal_sign('esi-context', $encoded);
        $token = $encoded . '.' . $signature;
        if (strlen($token) > 4096) {
            return new WP_Error('ultracache_esi_context_token_too_large', __('The ESI fragment context token exceeds the supported URL size.', 'ultracache'));
        }
    ...
    public function decode_context_token($token)
    {
        ...
        if ('' === $token || strlen($token) > 4096 || 1 !== preg_match('/^([A-Za-z0-9_-]+)\.([a-f0-9]{64})$/', $token, $matches)) {
            return new WP_Error('ultracache_esi_context_token_invalid', __('The ESI fragment token is invalid.', 'ultracache'));
        }

        $encoded = (string) $matches[1];
        $provided_signature = (string) $matches[2];
        if (!ultracache_internal_verify('esi-context', $encoded, $provided_signature)) {
            return new WP_Error('ultracache_esi_context_signature_invalid', __('The ESI fragment signature is invalid.', 'ultracache'));
        }

        $json = $this->base64url_decode($encoded);
        $payload = is_string($json) ? json_decode($json, true) : null;
        if (!is_array($payload) || 1 !== (int) ($payload['v'] ?? 0)) {
            return new WP_Error('ultracache_esi_context_payload_invalid', __('The ESI fragment payload is invalid.', 'ultracache'));
        }
        ...
        if (!is_string($locale) || strlen($locale) < 2 || strlen($locale) > 32) {
            $locale = function_exists('ultracache_varnish_esi_get_current_locale')
                ? ultracache_varnish_esi_get_current_locale()
                : (function_exists('get_locale') ? (string) get_locale() : 'en_US');
        }
    ...
    public function get_definition_summary($fragment_id)
    {
        ...
        return array(
            ...
            'maxRenderMs' => absint($definition['max_render_ms'] ?? 2000),
            ...
            'privateTransportDeclared' => 'private' === (string) ($definition['scope'] ?? '')
                && function_exists('ultracache_varnish_esi_private_definition_transport_is_declared')
    ...
    public function render(array $definition, array $context, $mode = 'renderer')
    {
        if ($this->render_depth >= 1) {
            return new WP_Error('ultracache_esi_nested_fragment_not_supported', __('Nested ESI fragment rendering is not supported in this version.', 'ultracache'));
        }
        ...
        if (!is_callable($callback)) {
            return new WP_Error('ultracache_esi_renderer_not_callable', __('The ESI fragment renderer is not callable.', 'ultracache'));
        }
        ...
        try {
            ...
            $render_duration_ms = max(0, (int) round((microtime(true) - $render_started_at) * 1000));
            $max_render_ms = max(10, min(5000, absint($definition['max_render_ms'] ?? 2000)));
            if ($render_duration_ms > $max_render_ms) {
                $this->render_depth--;
                do_action(
                    'ultracache_esi_fragment_render_timeout',
                    $definition['id'] ?? '',
                    $render_duration_ms,
                    $max_render_ms
                );
                return new WP_Error(
                    'ultracache_esi_fragment_render_timeout',
                    __('The ESI fragment renderer exceeded its configured time budget.', 'ultracache')
                );
            }
        } catch (Throwable $e) {
            while (ob_get_level() > $base_buffer_level) {
            ...
            return new WP_Error('ultracache_esi_fragment_render_failed', __('The ESI fragment renderer failed.', 'ultracache'));
        }
        $this->render_depth--;
    ...
    public function filter_cookie_header(array $definition, $cookie_header)
    {
        ...
        if (strlen($cookie_header) > $max_header_bytes) {
            return new WP_Error('ultracache_esi_private_cookie_header_too_large', __('The private ESI cookie header exceeds its configured limit.', 'ultracache'));
        }
        ...
        if (strlen($filtered_header) > $max_header_bytes) {
            return new WP_Error('ultracache_esi_private_cookie_header_too_large', __('The filtered private ESI cookie header exceeds its configured limit.', 'ultracache'));
        }
    ...
    private function bound_output($output, array $definition)
    {
        ...
        if (strlen($output) > $max_bytes) {
            return new WP_Error('ultracache_esi_fragment_output_too_large', __('The ESI fragment output exceeds its configured limit.', 'ultracache'));
        }
