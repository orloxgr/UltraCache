...
trait Ultra_Cache_WP_Varnish_ESI_Endpoint_Trait
{
    /**
     ...
     * @return array
     */
    public function register_varnish_esi_query_var(array $query_vars)
    {
     ...
     * @return array|null
     */
    public function maybe_bypass_varnish_esi_fragment_main_query($posts, $query)
    {
        ...
        $decoded = Ultra_Cache_Varnish_ESI_Registry::instance()->decode_context_token(rawurldecode($token));
        ...
        $scope_enabled = 'private' === $scope
            ...
            : function_exists('ultracache_varnish_esi_is_enabled') && ultracache_varnish_esi_is_enabled();
        if (!$scope_enabled) {
            return $posts;
        }
     ...
     * @return void
     */
    public function maybe_serve_varnish_esi_fragment()
    {
        ...
        if (!in_array($method, array('GET', 'HEAD'), true)) {
            ...
            $this->send_varnish_esi_error_response(405);
        }

        $decoded = Ultra_Cache_Varnish_ESI_Registry::instance()->decode_context_token(rawurldecode($token));
        if (is_wp_error($decoded)) {
            do_action('ultracache_esi_endpoint_error', $decoded, 'token');
            $this->send_varnish_esi_error_response(404);
        }
        ...
        $request_state = $this->prepare_varnish_esi_fragment_request_context($definition, $locale, $language);
        if (is_wp_error($request_state)) {
            do_action('ultracache_esi_endpoint_error', $request_state, 'transport');
            $this->send_varnish_esi_error_response('private' === $scope ? 403 : 404);
        }

        $context_hash = Ultra_Cache_Varnish_ESI_Registry::instance()->get_context_hash((string) $definition['id'], $context);
        if ('HEAD' === $method) {
            $this->restore_varnish_esi_fragment_request_context($request_state);
            $this->send_varnish_esi_fragment_success_headers(
                $definition,
                $scope,
                $context_hash,
                0,
        ...
        try {
            ...
            $output = Ultra_Cache_Varnish_ESI_Registry::instance()->render($definition, $context, 'renderer');
        ...
        if (is_wp_error($output)) {
            ...
            $fallback = Ultra_Cache_Varnish_ESI_Registry::instance()->render($definition, $context, 'fallback');
        ...
        if (!is_wp_error($output) && is_string($output)) {
            $output = $this->neutralize_varnish_esi_fragment_output($output);
        }

        $this->restore_varnish_esi_fragment_request_context($request_state);

        if (is_wp_error($output) || !is_string($output) || strlen($output) > (int) ($definition['max_output_bytes'] ?? 262144)) {
            $this->send_varnish_esi_error_response(500);
        }
     ...
     * @return void
     */
    private function send_varnish_esi_fragment_success_headers(
        array $definition,
        $scope,
        $context_hash,
        $output_bytes,
     ...
     * @return string
     */
    private function neutralize_varnish_esi_fragment_output($output)
    {
     ...
     * @param string $language   Signed parent WPML language code.
     * @return array|WP_Error
     */
    private function prepare_varnish_esi_fragment_request_context(array $definition, $locale = '', $language = '')
    {
        $scope = 'private' === (string) ($definition['scope'] ?? '') ? 'private' : 'public';
        ...
        if ('' !== $language) {
            if (!function_exists('ultracache_wpml_is_active') || !ultracache_wpml_is_active()
                || !function_exists('ultracache_wpml_get_active_languages')) {
                return new WP_Error('ultracache_esi_wpml_language_unavailable', __('The signed ESI language context is unavailable.', 'ultracache'));
            }
            $active_languages = ultracache_wpml_get_active_languages();
            if (!isset($active_languages[$language])) {
                return new WP_Error('ultracache_esi_wpml_language_inactive', __('The signed ESI language is no longer active.', 'ultracache'));
            }
            $state['wpmlLanguage'] = $language;
            $state['wpmlPreviousLanguage'] = function_exists('ultracache_wpml_get_current_language')
        ...
        if ('private' === $scope) {
            ...
            $filtered = Ultra_Cache_Varnish_ESI_Registry::instance()->filter_cookie_header(
                $definition,
        ...
        if ('' !== (string) $state['wpmlLanguage']
            && (string) $state['wpmlPreviousLanguage'] !== (string) $state['wpmlLanguage']
            && function_exists('do_action')) {
            do_action('wpml_switch_language', (string) $state['wpmlLanguage']);
            $current_language = function_exists('ultracache_wpml_get_current_language')
                ? ultracache_wpml_get_current_language()
                : '';
            $state['wpmlLanguageSwitched'] = (string) $state['wpmlPreviousLanguage'] !== $current_language;
            if ((string) $state['wpmlLanguage'] !== $current_language) {
                $this->restore_varnish_esi_fragment_request_context($state);
                return new WP_Error('ultracache_esi_wpml_language_switch_failed', __('The signed ESI language context could not be activated.', 'ultracache'));
            }
        }
     ...
     * @return void
     */
    private function restore_varnish_esi_fragment_request_context(array $state)
    {
        ...
        if (!empty($state['wpmlLanguageSwitched']) && function_exists('do_action')) {
            $previous_language = isset($state['wpmlPreviousLanguage']) ? (string) $state['wpmlPreviousLanguage'] : '';
    ...
    /**
     * Serve a short-lived parent or fragment response used by Redetect Varnish Capabilities.
     *
    ...
    private function serve_varnish_private_esi_probe_response($token, $kind)
    {
        ...
        if ('fragment' === $kind) {
            ...
            $request_level = absint(ultracache_server_value('HTTP_X_ESI_REQUEST_LEVEL'));
            if ('1' !== $internal_marker || $request_level < 1) {
                $this->send_varnish_esi_error_response(403);
            }
     ...
     * @return void
     */
    private function send_varnish_esi_error_response($status)
    {
